...
export default async function PropertiesPage({ searchParams }: { searchParams?: Promise<{ operacao?: string; tipo?: string; cidade?: string; bairro?: string; quartos?: string; banheiros?: string; vagas?: string; valorMin?: string; valorMax?: string; codigo?: string; }>; }) {
  ...
  const valorMin = onlyNumbers(params?.valorMin); const valorMax = onlyNumbers(params?.valorMax); const codigo = String(params?.codigo || "").trim();
  ...
  return <main className="page"><Navbar /><section className="pageTop propertiesTop"><div className="container"><h1>{params?.tipo ? params.tipo : "Imóveis"}</h1><p>Confira oportunidades cadastradas pela Alexandrowitch Imobiliária e Administradora.</p><SearchPanel initial={params || {}} /></div></section><section className="section sectionLight pageResultsSection"><div className="container"><div className="resultsHeader"><div><span className="eyebrow">Resultado da busca</span><h2>{properties.length} imóvel{properties.length === 1 ? "" : "is"} encontrado{properties.length === 1 ? "" : "s"}</h2></div>{hasFilter && <a className="btnLight" href="/imoveis">Limpar pesquisa</a>}</div>{error && <p className="sectionText">Erro ao carregar imóveis: {error.message}</p>}{!error && properties.length === 0 && <div className="emptyLuxury"><h3>Nenhum imóvel encontrado com esses filtros.</h3><p>Altere cidade, bairro, valor ou tipo de imóvel para ampliar a busca.</p></div>}<div className="grid3 resultGrid">{properties.map((property) => <PropertyCard key={property.id} property={property} images={imagesByProperty.get(property.id) || []} />)}</div></div></section><Footer /></main>;
}
