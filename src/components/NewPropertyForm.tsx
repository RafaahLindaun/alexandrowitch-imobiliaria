...
export default function NewPropertyForm() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      if (imageInput.files && imageInput.files.length > 0) {
        imageUrls = await uploadFiles(imageInput.files, `${slug}/fotos`);
      }

      if (floorInput.files && floorInput.files.length > 0) {
        floorPlanUrls = await uploadFiles(floorInput.files, `${slug}/plantas`);
      }

      if (brokerInput.files && brokerInput.files[0]) {
        const [url] = await uploadFiles(brokerInput.files, `${slug}/corretor`);
  ...
  return (
    <form className="formGrid" onSubmit={handleSubmit}>
      ...
      <select className="select" name="status" defaultValue="Disponível">
      ...
      <input className="input" name="broker_name" placeholder="Nome do corretor" />
      <input className="input" name="broker_phone" placeholder="WhatsApp do corretor" defaultValue="11974005163" />
      ...
      <label className="input full">
        Fotos do imóvel
        <br />
        <span className="formHint">Selecione várias fotos. A primeira será capa.</span>
        <br />
      ...
      <label className="input full">
        Plantas do imóvel
        <br />
        <span className="formHint">Envie imagens das plantas, se houver.</span>
        <br />
