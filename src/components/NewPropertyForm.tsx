...
async function uploadFiles(files: File[], folder: string) {
  ...
  for (const file of files) {
    const fileWithWatermark = await addWatermarkToImage(file);
    const fileExt = fileWithWatermark.name.split(".").pop() || "jpg";
    ...
    const { error } = await supabase.storage
      ...
      .upload(fileName, fileWithWatermark);
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      condominium_fee: String(form.get("condominium_fee") || ""),
      iptu: String(form.get("iptu") || ""),
      ...
      <form className="smartEditorForm" onSubmit={handleSubmit}>
        ...
        <section className="editorSection">
          ...
          <div className="editorGrid">
            ...
            <label className="editorField">
              ...
              <input name="condominium_fee" value={draft.condominium_fee} onChange={(event) => updateDraft("condominium_fee", event.target.value)} placeholder="R$ 850 / mês" />
            </label>

            <label className="editorField">
              <span>IPTU</span>
              <input name="iptu" value={draft.iptu} onChange={(event) => updateDraft("iptu", event.target.value)} placeholder="R$ 320 / mês" />
            </label>
          </div>
        </section>
        ...
        <section className="editorSection">
          ...
          <div className="editorGrid">
            ...
            <label className="editorField"><span>Mobiliado</span><select name="furnished" value={draft.furnished} onChange={(event) => updateDraft("furnished", event.target.value)}><option value="">Não informar</option><option value="Sim">Sim</option><option value="Não">Não</option></select></label>
          </div>
        </section>
