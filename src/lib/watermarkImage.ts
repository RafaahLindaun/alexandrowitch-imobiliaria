...
export async function addWatermarkToImage(file: File) {
  ...
  try {
    ...
    const desiredWidth = Math.min(
      Math.max(width * 0.16, 110),
      Math.min(width * 0.30, 280)
    );
    const desiredHeight = desiredWidth * (watermarkNaturalHeight / watermarkNaturalWidth);

    const margin = Math.max(width * 0.025, 18);
    const x = width - desiredWidth - margin;
    const y = height - desiredHeight - margin;
